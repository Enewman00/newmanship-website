...
#### Task 6.4: Cross-browser & device testing ✅ DONE (2026-05-12)
...
- Note: Full code-level cross-browser audit conducted. All JS APIs used (IntersectionObserver, localStorage, classList, URLSearchParams, history.replaceState, matchMedia) are supported in all modern browsers (Chrome 88+, Firefox 89+, Safari 15+, Edge 88+). Astro's `<ViewTransitions />` gracefully degrades in browsers without native View Transitions API support. Three bugs found and fixed: (1) `bg-brand-background` typo in `AppLandingLayout.astro` features section — the Tailwind color key is `brand-bg`, not `brand-background`; corrected to `bg-brand-bg`; (2) `transition-gap` in `AppCard.astro` — not a valid Tailwind v3 built-in, was silently stripped at build time; corrected to the arbitrary syntax `transition-[gap]` so the "Learn more" arrow gap animates correctly on hover; (3) missing `dark:bg-[#1C1C1F] dark:hover:bg-[#252528]` on the "Next →" post navigation card in `BlogLayout.astro` — the previous post card had dark variants but next did not; added to match. Font loading: `font-display: swap` + preloads for all 3 Inter weights eliminates FOUT. `backdrop-filter: blur()` in header: Tailwind v3 generates both `-webkit-backdrop-filter` and `backdrop-filter` prefixes automatically. `details/summary` FAQ: uses both `[list-style:none]` and `[&::-webkit-details-marker]:hidden` covering all browsers. Responsive layout uses `sm:`, `md:`, `lg:` breakpoints throughout; mobile-first grid and flex layouts tested at all breakpoints. `prefers-reduced-motion` disables all reveal animations and smooth scroll.
